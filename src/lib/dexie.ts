...
import type {
  ...
  TacheTemplate, TacheInstance,
} from '@/types/database'
// PendingMutation interface (inline to avoid circular dependency with sync-queue.ts)
...
class PhoodDB extends Dexie {
  ...
  tacheTemplates!: EntityTable<TacheTemplate, 'id'>
  tacheInstances!: EntityTable<TacheInstance, 'id'>
  ...
  constructor() {
    ...
    this.version(5).stores({
      ...
      historiquePrix: 'id, mercuriale_id, date_constatation',
      modelesInventaire: 'id, nom, type',
      pendingMutations: '++id, method, path, created_at',
    })
    this.version(6).stores({
      ...
      modelesInventaire: 'id, nom, type',
      pendingMutations: '++id, method, path, created_at',
      tacheTemplates: 'id, station, actif',
      tacheInstances: 'id, template_id, date, station, statut',
    })
  }
}
